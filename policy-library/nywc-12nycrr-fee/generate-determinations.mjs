...
// THE POPULATION BLOCK, derived from this file's own header and carried with the set. figure.mjs
// checks its digest against the live header at every render, so a figure over a set generated
// under different parameters cannot be rendered as if it were this one.
const params = parametersFromGeneratorHeader();
const population = { $source: 'the POPULATION PARAMETERS block of generate-determinations.mjs, one copy; this is a derived copy checked by digest', seed: SEED, count: N, parameters_sha256: params.sha256, parameters_text: params.text };
const out = { $note: 'SYNTHETIC. Generated by generate-determinations.mjs from the declared fact kinds and the population parameters in its header. No real claim, provider, name or scraped record. Results are never stored beside the facts; replay.mjs computes them.', seed: SEED, count: N, population, determinations };
