...
const n = det.length;
const pct = (k) => `${k}/${n} (${((100 * k) / n).toFixed(1)}%)`;
const text = [
...
console.log(text);
writeFileSync(`${HERE}/out/defensibility.json`, JSON.stringify({ $derived_by: 'defensibility.mjs', denominator: n, no_applied_bound: noBound, cites_version_not_in_force: { union, cites_other_register_version: citesOtherVersion, edition_not_in_force_on_dos: editionNotInForce, both }, dos_undetermined: dosUndetermined, no_edition_cited: noEdition, rendered: text, rows }, null, 1) + '\n');
